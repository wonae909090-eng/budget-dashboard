...
st.caption(
    "⚠️ 회귀 모델(추천예산 계산 자체)은 최신 달까지 포함한 전체 기간 데이터로 학습되며, "
    "최근 달일수록 더 큰 가중치를 둬 최근 트렌드에 더 민감하게 반응합니다. "
    "아래 표의 '전년동월 집행비용'과 '최근3개월 평균 집행비용'(회색 음영)은 계산 결과가 아니라, "
    "그 추천이 과거 대비 얼마나 큰 변화인지 보여주는 비교용 숫자일 뿐입니다. 실제로 확인해야 할 값은 "
    "'시나리오예산 / 예상DB수 / 예상DB단가'입니다."
)
...
with st.expander("📅 캠페인별 계절성 인사이트 (데이터 기반 자동 감지)"):
    st.caption(
        ...
        "**DA반영여부** = 같은 핵심 채널(DA 채널) 예산이 늘수록 단가가 더 가파르게 오르는 효과를 "
        "회귀 변수로 반영했는지 여부. 검증 결과 실제로 정확도가 개선될 때만 자동으로 반영됩니다."
    )
...
with st.expander("❓ 보수 / 중립 / 적극, 어떻게 다른가요?"):
    ...
    st.markdown(
        "**오해하기 쉬운 부분**: 이 세 시나리오는 \"작년 데이터를 더 참고하느냐 최근 트렌드를 더 참고하느냐\"의 "
        "차이가 아닙니다. **회귀 모델 자체는 세 시나리오 모두 완전히 동일**합니다(최근 달에 더 큰 가중치를 주는 "
        "것도 세 시나리오 모두 똑같이 적용됨 — 시나리오 간 차이가 아니라 회귀 모델 자체의 특성입니다). "
        "\"전년동월\"이 시나리오별 계산식에 쓰이는 건 **보수 시나리오뿐**입니다."
    )
    axis_df = pd.DataFrame([
        {"시나리오": "보수", "전년동월/최근트렌드를 계산에 쓰나": "예 (±10% 기준점)", "실제 차이 나는 지점": "변화 폭을 작게 제한"},
        {"시나리오": "중립", "전년동월/최근트렌드를 계산에 쓰나": "아니오", "실제 차이 나는 지점": "캠페인 간 배분을 물량(DB수) 기준으로"},
        {"시나리오": "적극", "전년동월/최근트렌드를 계산에 쓰나": "아니오", "실제 차이 나는 지점": "캠페인 간 배분을 효율(DB단가) 기준으로"},
    ])
    st.dataframe(axis_df, width="stretch", hide_index=True)
    compare_rows = []
...
for tab, name in zip(tabs, ["보수", "중립", "적극"]):
    with tab:
        ...
        st.markdown("**캠페인별 시나리오예산 (회색 음영 = 비교 참고용, 계산 결과 아님)**")
        display_table = table.rename(
            columns={"참고예산": "전년동월 집행비용", "최근3개월평균": "최근3개월 평균 집행비용"}
        )
        main_cols = [
            "캠페인구분", "전년동월 집행비용", "최근3개월 평균 집행비용",
            "시나리오예산", "예상DB수", "예상DB단가", "모델신뢰도(R2)",
        ]
        styled_table = (
            style_campaign_rows(
                display_table[main_cols],
                neutral_cols=["전년동월 집행비용", "최근3개월 평균 집행비용"],
            )
            .format(
                {
                    "전년동월 집행비용": "{:,.0f}",
        ...
        chart_df = display_table.melt(
            id_vars="캠페인구분",
            value_vars=["전년동월 집행비용", "최근3개월 평균 집행비용", "시나리오예산"],
        ...
        fig = px.bar(
            ...
            title=f"[{name}] 캠페인별 전년동월·최근3개월 평균 집행비용 vs 시나리오예산",
        )
